    ...
    return (
        <div className='flex flex-col items-center gap-4 my-16 text-[#262626] md:mx-10'>
            ...
            {doctors.length > 0 ? (
                <div className='w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
                    {doctors.slice(0, 10).map((item, index) => (
                        <div 
                            ...
                            key={index}
                            aria-label={`Book an appointment with Dr. ${item.name}`}
                        >
                            <img 
                ...
                <p className="text-gray-500 mt-4">No doctors available at the moment.</p>
            )}

            {/* "More" button */}
            <button 
                onClick={() => { navigate('/doctors'); window.scrollTo(0, 0); }} 
                className='bg-[#EAEFFF] text-gray-600 px-12 py-3 rounded-full mt-10 hover:scale-105 transition-all'
            >
                More
            </button>
        </div>
